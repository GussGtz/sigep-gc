    ...
    <main class="pt-14 pb-20 page-enter">
      ...
      <div class="bg-white border-b border-gray-100 px-5 py-5 lg:px-8">
        <div class="max-w-5xl mx-auto">
          <p class="text-xs text-gray-400 font-medium uppercase tracking-wider">Contabilidad · Glass Caribe</p>
          ...
          >
            ...
            <div class="p-4">
              ...
              <div class="flex gap-2">
                <button
                  v-if="getContabilidad(p) === 'pendiente'"
                ...
                >
                  ...
                  Iniciar en Contabilidad
                </button>

                <button
                  v-if="getContabilidad(p) === 'en proceso'"
                ...
                <span v-if="getContabilidad(p) === 'completado'"
                  ...
                  Contabilidad completada
                </span>
              </div>

              <!-- Nota de contabilidad: siempre visible si existe, con opción de editar -->
              <div class="mt-3 pt-3 border-t border-gray-50">
                <div v-if="getContabilidadNota(p) && !expandidos[p.id]" class="flex items-start gap-2 bg-blue-50 rounded-xl px-3 py-2 border border-blue-100">
                  ...
                  <p class="text-xs text-blue-700 flex-1">{{ getContabilidadNota(p) }}</p>
                  ...
                  <div v-if="expandidos[p.id]" class="mt-2 space-y-2">
                    <div class="flex gap-2">
                      <input
                        v-model="notas[p.id]"
                        type="text"
                        :placeholder="getContabilidadNota(p) || 'Escribe una nota para producción...'"
...
function getContabilidad(p)    { return p.areas?.find(a => a.area === 'contabilidad')?.estatus    || 'pendiente' }
function getContabilidadNota(p){ return p.areas?.find(a => a.area === 'contabilidad')?.comentarios || '' }

async function actualizarEstatus(p, nuevoEstatus) {
  updating.value[p.id] = true
  try {
    await pedidosStore.actualizarEstatus(p.id, 'contabilidad', nuevoEstatus)
    setTimeout(() => notifs.fetchFromDB(), 800)
    toast.add({ type: 'success', message: 'Estado de Contabilidad actualizado' })
  } catch {
...
async function guardarNota(p) {
  ...
  try {
    await pedidosStore.actualizarEstatus(p.id, 'contabilidad', getContabilidad(p), nota)
